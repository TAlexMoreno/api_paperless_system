...
const DATABASE_URL = `postgres://${process.env.POSTGRES_USER}:${process.env.POSTGRES_PASSWORD}@${POSTGRES_HOST}:${POSTGRES_PORT}/${process.env.POSTGRES_PAPERLESS_DB}`;
    
export default async function connectToPaperlessDatabase(): Promise<NodePgDatabase> {
